...
let connection = null;
let channel = null; // this will store the shared channel, needs to be global since more functions use it.
...
export const connectRabbitMQ = async () => {
    try {
        ...
        connection?.on("close", () => {
            console.warn(chalk.yellow("⚠️ RabbitMQ connection closed. Reconnecting..."));
...
export const publishToQueue = async (queueName, message) => {
    ...
    if (!channel) {
        console.warn(chalk.yellow("⚠️ RabbitMQ channel not ready. Message skipped."));
        return;
    }
